...
def discover_wavs(root: Path, max_duration_s: Optional[float] = 10.0) -> Tuple[List[Path], int, int]:
    """Recursively discover readable WAV files and optionally reject long files.

    Returns ``(accepted, skipped_too_long, skipped_unreadable)``. Duration is read
    from the WAV header with ``soundfile.info`` so long recordings are not loaded
    into memory just to decide whether to keep them.
    """
    accepted: List[Path] = []
    skipped_too_long = 0
    skipped_unreadable = 0

    for path in sorted(p for p in root.rglob("*") if p.is_file() and p.suffix.lower() == ".wav"):
        try:
    ...
    return accepted, skipped_too_long, skipped_unreadable
...
def build_app(config: AppConfig) -> Dash:
    ...
    wav_paths, skipped_long, skipped_unreadable = discover_wavs(root, config.max_duration_s)
    if not wav_paths:
        raise ValueError(
            f"No eligible WAV files found recursively in {root}. "
            f"Maximum duration is {config.max_duration_s:g} s."
        )
    ...
    app.layout = html.Div(
        [
            ...
            # IMPORTANT: viewport is intentionally separate from session-state.
            # Zoom/pan therefore does not cause the figure to be re-rendered.
            ...
            html.Div(
                [
                    ...
                    html.Div(
                        f"Eligible WAV: {len(wav_paths)} | > {config.max_duration_s:g} s skipped: {skipped_long} "
                        f"| unreadable skipped: {skipped_unreadable}"
                    ),
    ...
    def remember_view(relayout: Optional[Dict[str, Any]], view: Dict[str, Any], state: Dict[str, Any]):
        if not relayout:
            return no_update
        result = dict(view or {})
        current_rel = state["relative_path"]
        if result.get("relative_path") != current_rel:
            result = {"relative_path": current_rel}

        # Store only explicit axis ranges. Autorange/reset removes stale ranges.
        if relayout.get("xaxis.autorange"):
            result.pop("xaxis.range[0]", None)
            result.pop("xaxis.range[1]", None)
        if relayout.get("yaxis.autorange"):
            result.pop("yaxis.range[0]", None)
            result.pop("yaxis.range[1]", None)

        for key in ["xaxis.range[0]", "xaxis.range[1]", "yaxis.range[0]", "yaxis.range[1]"]:
            if key in relayout:
                result[key] = relayout[key]
        return result
    ...
    def controller(
        ...
        snap_values, db_floor, view_state, state,
    ):
        ...
        if trigger == "spectrogram":
            ...
            if "on" in (snap_values or []):
                view = view_state or {}
                same_file = view.get("relative_path") == state["relative_path"]
                xr = None
                yr = None
                if same_file and "xaxis.range[0]" in view and "xaxis.range[1]" in view:
                    xr = [float(view["xaxis.range[0]"]), float(view["xaxis.range[1]"])]
                if same_file and "yaxis.range[0]" in view and "yaxis.range[1]" in view:
                    yr = [float(view["yaxis.range[0]"]), float(view["yaxis.range[1]"])]
