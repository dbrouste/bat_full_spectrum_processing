...
def make_figure(spec, chirps, db_floor, db_max, uirevision, x_range=None, y_range=None):
    ...
    fig.add_trace(go.Heatmap(
        x=spec["times_ms"], y=spec["freqs_khz"], z=spec["db"],
        zmin=db_floor, zmax=db_max, colorscale="Viridis",
        colorbar=dict(title="dB rel."),
        hovertemplate="t=%{x:.3f} ms<br>f=%{y:.2f} kHz<br>%{z:.1f} dB<extra></extra>",
    ))
    fig.add_trace(go.Scatter(
        x=arrays["px"], y=arrays["py"], customdata=arrays["custom"], mode="markers",
    ...
    fig.add_trace(go.Scatter(
        x=arrays["lx"], y=arrays["ly"], mode="lines", line=dict(width=3),
        name="Chirp curves", hoverinfo="skip",
    ))
    fig.update_layout(
        margin=dict(l=60, r=20, t=35, b=55),
        xaxis_title="Temps (ms)", yaxis_title="Fréquence (kHz)",
...
def annotation_patch(chirps):
    a = annotation_arrays(chirps)
    patch = Patch()
    patch["data"][1]["x"] = a["px"]
    patch["data"][1]["y"] = a["py"]
    patch["data"][1]["customdata"] = a["custom"]
    patch["data"][2]["x"] = a["lx"]
    patch["data"][2]["y"] = a["ly"]
    return patch
...
def build_app(config: AppConfig) -> Dash:
    ...
    if not wav_paths:
        raise ValueError(f"No usable WAV files found recursively in: {root}.")
    ...
    def load_spec(rel):
        if rel not in spec_cache:
            spec_cache[rel] = compute_spectrogram(
                root / rel, config.fmin_khz, config.fmax_khz,
    ...
    floor_style = {"width": "40px", "minWidth": "40px", "maxWidth": "40px", "padding": "5px 3px", "fontSize": "15px"}
    ...
    app.layout = html.Div([
        ...
        html.Div([
            html.Div([
                html.Label("dB floor", style={"whiteSpace": "nowrap"}),
                dcc.Input(id="db-floor", type="number", value=config.db_floor, step=1, debounce=True, style=floor_style),
            ], style={"display": "flex", "alignItems": "center", "gap": "5px", "flex": "0 0 auto"}),
            html.Div([
                html.Label("dB max", style={"whiteSpace": "nowrap"}),
                dcc.Input(id="db-max", type="number", value=initial_state["db_max_default"], step=1, debounce=True, style=max_style),
            ], style={"display": "flex", "alignItems": "center", "gap": "5px", "flex": "0 0 auto"}),
            dcc.Checklist(
                id="snap-enabled",
        ...
        html.Div([
            ...
            html.Div(f"WAV accepted: {len(wav_paths)} | skipped > {config.max_duration_s}s: {skipped_long} | unreadable: {unreadable}"),
    ...
    def render_graph(_fr, _ar, db_floor, db_max, state, viewport):
        ...
        file_changed = "file-revision.data" in triggered
    ...
    @app.callback(
        ...
        Input("new-chirp", "n_clicks"), Input("add-point", "n_clicks"),
        Input("move-point", "n_clicks"), Input("delete-point", "n_clicks"),
    ...
    def controller(_new, _add, _move, _delete_point, _undo, _finish, _delete_chirp,
        ...
        if trigger == "spectrogram":
            ...
            if mode in {"move_select", "delete_point"}:
                ...
                pi = int(np.argmin([
                    ((p["t_ms"] - click_t) / t_span) ** 2 + ((p["f_khz"] - click_f) / f_span) ** 2
                    for p in points
                ]))
...
def run_annotator(
    ...
    cfg = AppConfig(
        root=folder, annotations=annotations, seed=seed,
        fmin_khz=fmin_khz, fmax_khz=fmax_khz,
        nperseg=nperseg, noverlap=noverlap,
        db_floor=db_floor, max_duration_s=max_duration_s,
    )
    app = build_app(cfg)
